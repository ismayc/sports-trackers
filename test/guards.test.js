...
describe('the storage namespace', () => {
  ...
  // (ismayc.github.io), so localStorage is shared. The hub is the switchboard:
  // it deliberately writes each viewer's OWN follow key, so unlike the viewers
  // it must name every sibling prefix. What it must not do is drift from them.
  const FAMILY = [
    ['pl:', 'premier-league'],
  ...
  it('keeps its own preferences under st:', () => {
    for (const file of files) {
      const src = read(file)
  ...
  it('registers every sibling viewer exactly once, and nothing else', () => {
    // A viewer missing here is invisible in the hub; a prefix that matches no
    // repo writes a follow key nothing ever reads.
    const src = read('src/data/viewers.js')
    const used = [...src.matchAll(/'([a-z0-9]+:)followed'/g)].map((m) => m[1])
    const siblings = FAMILY.map(([p]) => p).filter((p) => p !== OWN)
    expect([...used].sort()).toEqual([...siblings].sort())
    expect(used).not.toContain(OWN)
  })
})
